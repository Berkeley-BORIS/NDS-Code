%%%PARSE AND SAVE DATA 9/12 - no longer using BUTTONS now using MESSAGES
...
while(1)
    ...
    if( cnt >= start_line ) % skip header, start at this line 
        if( isempty(strfind( L, '.....' )) == 0 || isempty(strfind( L, '...C.' )) == 0) %the dots indicate that the data in this line have good corneal reflections, ignore lines with no CRs or interpolated
