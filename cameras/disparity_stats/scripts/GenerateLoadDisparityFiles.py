...
src_tail = '3d_reprojection/data/' + subj + fixation_flag + '/' + subj + '_' + activity + '_eye_images_transform1/10deg/disparityeye/'
src_hd1 = '../../'
...
create_disparity_stats_matrix(subj=subj, activity=activity, src_dir=src_dir,dst_dir=dst_dir,start_frame=start_frame,duration_sec=duration_sec)""".format(exp_id, flag, 'task_' + activity)

            fname = '_'.join([exp_id + flag, 'load_disparity_stats', activity]) + '.py'
